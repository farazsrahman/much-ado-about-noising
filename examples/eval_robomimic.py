...
import numpy as np
import torch
# from torch.optim.lr_scheduler import CosineAnnealingLR  # training only
...
# from mip.dataset_utils import loop_dataloader  # noqa: E402  # training only
...
# from mip.logger import compute_average_metrics, update_best_metrics  # training only
from mip.samplers import get_default_step_list  # noqa: E402
# from mip.scheduler import WarmupAnnealingScheduler  # noqa: E402  # training only
...
def train(config: Config, envs, dataset, agent, logger, resume_state=None):
    ...
    """
    # ---------- TRAINING-SPECIFIC CODE (commented out for eval-only script) ----------
    # # dataloader
    ...
    #         agent.train()
    pass  # eval-only: train() is a no-op
...
def main(config):
    ...
    # dataset setup
    #
    # Eval does not require loading RGB frames from the dataset file. We only need:
    # - lowdim/action min-max normalizers
    # - fixed ImageNormalizer transform (no dataset stats)
    ...
    # resume_state = None  # training only
    ...
    if config.optimization.model_path and config.optimization.model_path != "None":
        ...
        agent.load(config.optimization.model_path, load_optimizer=False)  # eval: no optimizer
    # elif config.optimization.auto_resume:  # training only
    ...
    # if config.mode == "train":  # training branch commented out for eval-only script
